...
public class ScanPreviewActivity extends AppCompatActivity {
    ...
    private void initCamera() {
        ...
        cameraManager.setOnCameraAnalyserCallback((bitmap, barcodes) -> {
    ...
    private void initConfig() {
        mScanConfig = (ScanConfig) getIntent().getSerializableExtra(ScanManager.INTENT_KEY_CONFIG_MODEL);
        if (mScanConfig == null) {
            mScanConfig = new ScanConfig.Builder().builder();
        }
    }

    private void initViews() {
        rl_act_root = findViewById(R.id.rl_act_root);
        ...
        action_menu_view = findViewById(R.id.action_menu_view);
        result_point_view = findViewById(R.id.result_point_view);
        ...
        action_menu_view.setScanConfig(mScanConfig, ScanConfig.mCustomViewBindCallback);
    }
            ...
            new Thread(new Runnable() {
                ...
                public void run() {
                    ...
                    cameraManager.getBarcodeAnalyser().getBarcodeScanner().process(inputImage)
                            .addOnSuccessListener(barcodes -> runOnUiThread(new Runnable() {
                                @Override
                                public void run() {
                                    Log.i("ScanPreviewActivity", "barcodes.size():" + barcodes.size());
                                    if (barcodes.isEmpty()) {
                                        cameraManager.setAnalyze(true);
...
//                                                finishSuccess(barcodes.get(0).getRawValue());
//                                            }else {
//                                                result_point_view.setDatas(barcodes, decodeAbleBitmap);
//                                                result_point_view.setVisibility(View.VISIBLE);
//                                            }
    ...
    private void finishCancle() {
        setResult(ScanManager.RESULT_CANCLE, new Intent());
        finishFinal();
    }

    private void finishFailed(String errorMsg) {
        Intent intent = new Intent();
        intent.putExtra(ScanManager.INTENT_KEY_RESULT_ERROR, errorMsg);
        setResult(ScanManager.RESULT_FAIL, intent);
        finishFinal();
    }
    ...
    private void finishSuccess(ArrayList<String> results) {
        Intent intent = new Intent();
        intent.putStringArrayListExtra(ScanManager.INTENT_KEY_RESULT_SUCCESS, results);
        setResult(ScanManager.RESULT_SUCCESS, intent);
        finishFinal();
    }
